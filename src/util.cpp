...
const char * const BITCOIN_CONF_FILENAME = "unode.conf";
const char * const BITCOIN_PID_FILENAME = "unoded.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "unode" is a composite category enabling all unode-related debug output
            if(ptrCategory->count(string("unode"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\unodeCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\unodeCore
    // Mac: ~/Library/Application Support/unodeCore
    // Unix: ~/.unodecore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "unodeCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/unodeCore";
#else
    // Unix
    return pathRet / ".unodecore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty unode.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override unode.conf
